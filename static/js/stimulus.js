...
(function () {
  ...
  function loadLetters() {
    ...
    return window.__LETTERS__ || [];
  }
  const LETTERS = loadLetters();
  const MAX_LIVES = 3;
  const SHOP_ITEMS = [{
    key: "skincare_kit",
    name: "Glow Essence Kit",
    price: 60,
    emoji: "🧴"
  }, {
    key: "tumbler",
    ...
    key: "makeup_palette",
    name: "Pastel Makeup Set",
    price: 45,
    emoji: "🎨"
  }, {
    key: "plushie",
    name: "Cuddly Bear Toy",
    price: 30,
    emoji: "🧸"
  }, {
    key: "heels",
    name: "Starlight Shoes",
    price: 85,
    emoji: "👠"
  }, {
    key: "gown",
  ...
  const Game = {
    ...
    saveProgress(host) {
      ...
      const usedIds = this.letters.map((l, i) => ({
        id: l.id,
        used: !this.unused.includes(i) && this.currentIndex !== i
    ...
    showStart() {
      ...
      if (loggedIn && auth) {
        ...
        actions.appendChild(el("button", {
          class: "wood-btn",
          text: "Set up a profile",
          on: {
            click: () => auth && auth.openSignUp(() => this.beginStory(false))
          }
        }));
        actions.appendChild(el("button", {
          class: "wood-btn",
          text: "Sign in",
          on: {
            click: () => auth && auth.openSignIn(() => this.beginStory(false))
          }
        }));
      }
      const stack = el("div", {
    ...
    // ========================= LOBBY =========================
    showLobby() {
      if (!this.unused.length && this.currentIndex === null) {
        this.showEnding();
        return;
      }
      const screen = el("div", {
      ...
      const letterBtn = el("button", {
        class: "wood-btn plaque",
        on: {
          click: () => this.showCurrentLetter()
        }
      }, el("span", {
        class: "ico",
        text: "✉️"
      ...
      const wardrobeZone = el("div", {
        class: "wardrobe-zone"
      });
      const wardrobe = el("div", {
        class: "wardrobe-panel"
      });
      const renderWardrobe = () => {
        wardrobe.innerHTML = "";
        wardrobe.appendChild(el("div", {
          class: "wardrobe-title",
          text: "Wardrobe"
        }));
        const list = el("div", {
          class: "wardrobe-list"
      ...
      const hangerSvg = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" ' + 'stroke-linecap="round" stroke-linejoin="round">' + '<path d="M12 8.2V9.6"/>' + '<path d="M10.5 6.1a1.6 1.6 0 1 1 1.5 1.9"/>' + '<path d="M12 9.6 3.1 15.5c-.8.53-.42 1.8.55 1.8h16.7c.97 0 1.35-1.27.55-1.8L12 9.6Z"/>' + '</svg>';
      const fab = el("button", {
        class: "wardrobe-fab",
        html: hangerSvg,
        attrs: {
    ...
    showCurrentLetter() {
      if (this.currentIndex === null) {
        ...
        this.currentIndex = this.unused.pop();
      }
      ...
      const after = el("div", {
        ...
        class: "wood-btn",
        text: "See your choices",
        on: {
          click: () => this.showChoices()
        }
      }));
      let revealed = false;
      const revealButton = () => {
      ...
      const paperFallback = el("div", {
        class: "letter-paper letter-fallback"
      }, el("h3", {
      ...
      if (ASSETS.letterVideo) {
        ...
        const video = el("video", {
          ...
          attrs: {
            ...
            playsinline: "",
            // iOS: play inline instead of fullscreen
            preload: "auto"
          }
        });
        ...
        video.addEventListener("loadedmetadata", () => {
          const ms = isFinite(video.duration) && video.duration > 0 ? video.duration * 1000 + 400 : 12000;
    ...
    showChoices() {
      ...
      const cardMeta = [{
        title: "Hard",
        body: "Play the logigrame, test your skills and earn 50 coins.",
        pid: 1,
        dataIndex: 0
      }, {
        title: "Easy",
        body: "Play the sequence game, test your skills and earn 30 coins.",
        pid: 2,
        dataIndex: 1
      }];
      const screen = el("div", {
      ...
      stage.appendChild(el("button", {
        class: "wood-btn exit-top",
        text: "Exit to Lobby",
        on: {
          click: () => this.finishLetter()
        }
      }));
      ...
      const cards = cardMeta.map(m => {
        const card = el("div", {
          ...
          text: "Solve Puzzle",
          on: {
            click: () => this.launchPuzzle(m.pid, letter, m.dataIndex)
          }
        }));
    ...
    // ====================== PUZZLE LAUNCH ====================
    launchPuzzle(pid, letter, cardIdx) {
      if (pid === 1) {
    ...
    showShop() {
      ...
      const status = el("div", {
        class: "shop-status",
        text: "A few curated essentials."
      });
      const grid = el("div", {
        class: "shop-grid"
      });
      const refresh = () => {
        badge.querySelector(".coin-count").textContent = this.coins.toLocaleString();
        [...grid.children].forEach(node => {
          const elNode = node;
          const key = elNode.dataset.effect;
          ...
          const pill = elNode.querySelector(".price-pill");
          pill.innerHTML = owned ? "Owned" : `<span class="mini-coin"></span>${price}`;
        });
      };
      SHOP_ITEMS.forEach(item => {
        const node = el("div", {
          ...
          class: "price-pill",
          html: `<span class="mini-coin"></span>${item.price}`
        }));
    ...
    showEnding() {
      ...
      screen.appendChild(el("div", {
        ...
        text: "Eve's story, for now"
      }), el("p", {
        text: "Every letter has been read. The room looks different — not to anyone else, but rooms remember, and yours is remembering. You showed up. The tree, insufferably, noticed too."
      }), el("div", {
  ...
  function burstConfetti(canvas, won, ms) {
    ...
    function frame(now) {
      ...
      if (now - t0 < ms) raf = requestAnimationFrame(frame);else {
        stop = true;
        ctx.clearRect(0, 0, canvas.width, canvas.height);
  ...
  function endOverlay(host, won, coins, onDone) {
    const fx = $("#fx");
    burstConfetti(fx, won, 5000);
    const card = won ? el("div", {
      class: "overlay-card"
    }, el("div", {
      class: "overlay-coin",
      text: "C"
      ...
      class: "sub",
      text: "Enjoy a little shopping…"
    })) : el("div", {
      class: "overlay-card loss"
    }, el("div", {
      class: "broken-heart",
      text: "💔"
    }), el("h2", {
      ...
      class: "sub",
      text: "Give it another try and earn some coins."
    }));
    const ov = el("div", {
      class: "overlay show"
    }, card);
  ...
  // ============================================================
  const Logigrame = {
    open(data, coins, returnCb) {
      const subjects = data.subjects;
      const attributes = data.attributes;
      const options = data.options;
      const solution = {};
      for (const k in data.solution) {
        const t = parseSolKey(k);
        if (t) solution[t[0] + "||" + t[1]] = data.solution[k];
      }
      const filled = {};
      let lives = MAX_LIVES,
        gameOver = false,
      ...
      const header = el("div", {
        ...
        text: "Exit",
        on: {
          click: () => finish(false, false)
        }
      }));
      const status = el("div", {
        class: "status-line",
        text: "Read the clues, then click an empty slot to fill it in."
      });
      ...
      subjects.forEach(s => thead.appendChild(el("th", {
        text: s
      })));
      ...
      // clues
      const clueGrid = el("div", {
        class: "clue-grid"
      });
      (data.clues || []).forEach(c => {
        clueGrid.appendChild(el("div", {
          class: "clue-row"
        }, el("div", {
          class: "ico",
          text: c[0]
      ...
      const left = el("div", {
        class: "logi-left"
      }, gridCard, status, clueCard);

      // choice panel
      const cpTitle = el("div", {
        class: "cp-title",
        text: "SELECT CELL"
      });
      const cpDesc = el("div", {
        class: "cp-desc",
        text: "Click an empty slot on the grid to choose a value."
      });
      const cpBtns = el("div", {});
      const choicePanel = el("div", {
        class: "choice-panel"
      }, cpTitle, cpDesc, cpBtns);
      const body = el("div", {
        class: "logi-body"
      }, left, choicePanel);
      ...
      screen.appendChild(shell);
      Game.mount(screen);
      function usedValues(attr) {
        const set = new Set();
        for (const k in filled) {
          const parts = k.split("||");
          if (parts[1] === attr) set.add(filled[k]);
        }
        return set;
      }
      function clearPanel() {
      ...
      function cellClick(subj, attr, cell) {
        ...
        options[attr].forEach(val => {
          ...
          if (used.has(val)) b.disabled = true;else b.addEventListener("click", () => pick(subj, attr, val, cell));
          cpBtns.appendChild(b);
        });
        status.textContent = `Choose a value for ${subj} (${attr}).`;
      }
      function pick(subj, attr, val, cell) {
  ...
  const Sequence = {
    open(data, coins, returnCb) {
      ...
      const header = el("div", {
        ...
        class: "ptitle",
        text: (data.title || "IN ORDER").toUpperCase()
      }), el("span", {
        class: "spacer"
      }), hearts, el("button", {
        class: "exit-mini",
      ...
      const status = el("div", {
        class: "status-line",
        text: "Drag the tiles into the order that feels right."
      });
      const verify = el("button", {
        class: "wood-btn",
        text: `Verify · +${coins} coins`,
        on: {
          click: check
        }
      });
          ...
          if (!locked) {
            ...
            tile.addEventListener("click", () => {
              ...
              if (tapSel === null) {
                ...
                const tmp = order[tapSel];
                order[tapSel] = order[pos];
                order[pos] = tmp;
                tapSel = null;
                render();
              }
      ...
      function check() {
        ...
        if (order.every((v, i) => v === correctOrder[i])) {
          ...
          if (lives <= 0) {
            ...
            setTimeout(() => {
              gameOver = true;
              endOverlay(screen, false, 0, () => finish(false, false));
            }, 700);
          } else {
