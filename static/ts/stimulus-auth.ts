...
(function () {
  ...
  const Auth: AuthState = {
    ...
    loggedIn(): boolean { return !!this.access; },
  };

  async function api(url: string, method: string, body?: any): Promise<any> {
  ...
  const Progress: ProgressApi = {
    ...
    async save(state: ProgressState): Promise<any> {
  ...
  // ---- small helpers ----
  function makeEl(tag: string, opts: ElOpts = {}, ...kids: Array<Node | null>): HTMLElement {
  ...
  // ============================================================
  function showWelcome(name: string | null, done?: () => void): void {
    const embers = makeEl("div", { class: "welcome-embers" });
    for (let i = 0; i < 7; i++) {
      const e = makeEl("span", {});
      e.style.left = (6 + i * 13) + "%";
    ...
    const ov = makeEl("div", { class: "welcome-overlay" },
      embers,
      makeEl("div", { class: "welcome-card" },
        makeEl("div", { class: "welcome-mark", text: "✦" }),
        makeEl("div", { class: "welcome-eyebrow", text: "Your room is ready" }),
        makeEl("div", { class: "welcome-title", text: "Welcome" }),
        makeEl("div", { class: "welcome-name",
          text: name ? "to your story, " + name + "." : "to your story." }),
        makeEl("div", { class: "welcome-rule" })
      )
    );
  ...
  // ============================================================
  function buildAuthModal(mode: "signin" | "signup", onSuccess?: (created: boolean) => void): void {
    const app = document.getElementById("app") as HTMLElement;
    const overlay = makeEl("div", { class: "auth-overlay show" });

    const errBox = makeEl("div", { class: "auth-error" });
    const userInput = makeEl("input", { attrs: { type: "text", autocomplete: "username" } }) as HTMLInputElement;
    const passInput = makeEl("input", { attrs: { type: "password", autocomplete: "current-password" } }) as HTMLInputElement;
    const nameInput = makeEl("input", { attrs: { type: "text", autocomplete: "nickname" } }) as HTMLInputElement;

    let current: "signin" | "signup" = mode;

    const title = makeEl("h2", {});
    const sub = makeEl("div", { class: "auth-sub" });
    const nameField = makeEl("div", { class: "auth-field" },
      makeEl("label", { text: "Display name" }), nameInput);
    const submitBtn = makeEl("button", { class: "wood-btn auth-fullwidth" }) as HTMLButtonElement;
    const switchLine = makeEl("div", { class: "auth-switch" });

    function render(): void {
      const signup = current === "signup";
      title.textContent = signup ? "Create profile" : "Welcome back";
      sub.textContent = signup
        ? "Save your coins, your shop, and your letters."
      ...
      switchLine.innerHTML = signup
        ? `Already have a profile? <a>Sign in</a>`
        : `New here? <a>Create a profile</a>`;
      (switchLine.querySelector("a") as HTMLElement).addEventListener("click", () => {
        current = signup ? "signin" : "signup"; errBox.textContent = ""; render();
      });
    }
    ...
    async function submit(): Promise<void> {
      ...
      try {
        if (current === "signup") {
          ...
          await api(API.register, "POST", { username, password, display_name: displayName });
        }
        const tok = await api(API.login, "POST", { username, password });
        // fetch display name
        Auth.setTokens(tok.access, tok.refresh, username);
        try {
          const me = await api(API.me, "GET");
          Auth.name = me.display_name; localStorage.setItem("stim_name", me.display_name);
        } catch (e) { /* keep username */ }
        overlay.remove();
        const created = current === "signup";
        showWelcome(Auth.name || username, () => { if (onSuccess) onSuccess(created); });
      } catch (e: any) {
    ...
    passInput.addEventListener("keydown", (e: KeyboardEvent) => { if (e.key === "Enter") submit(); });

    const card = makeEl("div", { class: "auth-card" },
      makeEl("button", { class: "auth-close", html: "&times;", on: { click: () => overlay.remove() } }),
      title, sub,
      errBox,
      nameField,
      makeEl("div", { class: "auth-field" }, makeEl("label", { text: "Username" }), userInput),
      makeEl("div", { class: "auth-field" }, makeEl("label", { text: "Password" }), passInput),
      makeEl("div", { class: "auth-actions" }, submitBtn),
      switchLine
    );
  ...
  window.StimulusAuth = {
    Auth, Progress,
    openSignIn(onSuccess?: (created: boolean) => void) { buildAuthModal("signin", onSuccess); },
    openSignUp(onSuccess?: (created: boolean) => void) { buildAuthModal("signup", onSuccess); },
    loggedIn() { return Auth.loggedIn(); },
    displayName() { return Auth.name; },
    logout() { Auth.clear(); },
    welcome(name: string | null, done?: () => void) { showWelcome(name, done); },

    // Build the small "signed in as" chip with a logout control.
    userChip(onLogout?: () => void): HTMLElement | null {
      if (!Auth.loggedIn()) return null;
      return makeEl("div", { class: "auth-userchip" },
        makeEl("span", { text: "👤 " + (Auth.name || "Player") }),
        makeEl("span", { class: "logout-x", text: "⏻", attrs: { title: "Sign out" },
          on: { click: () => { Auth.clear(); if (onLogout) onLogout(); } } })
      );
    },

    // Show a brief "Progress saved" toast.
    toast(host: HTMLElement, message?: string): void {
      const t = makeEl("div", { class: "save-toast", text: message || "Progress saved" });
      host.appendChild(t);
